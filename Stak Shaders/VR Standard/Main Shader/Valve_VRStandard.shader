...
Shader "Valve/VRStandard"
{
	Properties
	{
		...
		[HideInInspector][Toggle]_ZWrite("ZWrite", Float) = 1
		[HideInInspector]_BlendSrc("Blend Source", Float) = 1
		[HideInInspector]_Surface("Surface", Int) = 0
	...
	SubShader
	{
		...
		Pass
		{
				...
				) : SV_Target
			{
				...
				float4 uvs4_ParallaxMap = float4(i.uv0XY_bitZ_fog.xy,0,0);
				uvs4_ParallaxMap.xy = float4(i.uv0XY_bitZ_fog.xy,0,0).xy * _ParallaxMap_ST.xy + _ParallaxMap_ST.zw;
				...
				float2 OffsetPOM153 = POM( _ParallaxMap, uvs4_ParallaxMap.xy, ddx(uvs4_ParallaxMap.xy), ddy(uvs4_ParallaxMap.xy), ase_worldNormal, ase_worldViewDir, ase_tanViewDir, 8, 8, temp_output_155_0, 0, _ParallaxMap_ST.xy, float2(0,0), 0 );
			...
			//---------------------------------------------------------------------------------------------------------------------------
				...
				half3 emissionbaked = ( temp_output_249_0 * _BakedMutiplier ).rgb;
			...
			// End Injection NORMAL_MAP from Injection_NormalMaps.hlsl ----------------------------------------------------------
				...
				half alpha = ( staticSwitch305 + ( tex2DNode9.a * _Color.a ) + ( ( _Surface * _BlendDst * _BlendSrc * _ZWrite * _Cull ) * 0.0 ) );
		...
		Pass
		{
			     ...
			     ) : SV_Target
			{
			    ...
			    float staticSwitch305 = 0.0;
			    #endif
			    float4 uvs4_ParallaxMap = i.ase_texcoord;
			    uvs4_ParallaxMap.xy = i.ase_texcoord.xy * _ParallaxMap_ST.xy + _ParallaxMap_ST.zw;
			    float temp_output_155_0 = ( _Parallax * -1.0 );
		...
		Pass
		{
				...
				) : SV_Target
			{
			   ...
			   float3 unpack99 = UnpackNormalScale( tex2D( _DetailNormalMap, OffsetPOM398 ), _DetailNormalMapScale );
			   unpack99.z = lerp( 1, unpack99.z, saturate(_DetailNormalMapScale) );
			   float4 uvs4_ParallaxMap = float4(i.uv0.xy,0,0);
			   uvs4_ParallaxMap.xy = float4(i.uv0.xy,0,0).xy * _ParallaxMap_ST.xy + _ParallaxMap_ST.zw;
			   float2 OffsetPOM153 = POM( _ParallaxMap, uvs4_ParallaxMap.xy, ddx(uvs4_ParallaxMap.xy), ddy(uvs4_ParallaxMap.xy), ase_worldNormal, ase_worldViewDir, ase_tanViewDir, 8, 8, temp_output_155_0, 0, _ParallaxMap_ST.xy, float2(0,0), 0 );
			   float clampResult73 = clamp( _BumpScale , 0.0 , 1.0 );
			   float3 unpack70 = UnpackNormalScale( tex2D( _BumpMap, ( float2( 0,0 ) + OffsetPOM153 ) ), clampResult73 );
			   unpack70.z = lerp( 1, unpack70.z, saturate(clampResult73) );
		...
		Pass
		{
			    ...
			    ) : SV_TARGET
			{
			    ...
			    float staticSwitch305 = 0.0;
			    #endif
			    float4 uvs4_ParallaxMap = input.ase_texcoord1;
			    uvs4_ParallaxMap.xy = input.ase_texcoord1.xy * _ParallaxMap_ST.xy + _ParallaxMap_ST.zw;
			    float temp_output_155_0 = ( _Parallax * -1.0 );
		...
		Pass
		{
			...
			half4 frag(v2f i  ) : SV_Target
			{
				UNITY_SETUP_INSTANCE_ID(i);
				UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(i);
				float4 uvs4_ParallaxMap = float4(i.uv,0,0);
				uvs4_ParallaxMap.xy = float4(i.uv,0,0).xy * _ParallaxMap_ST.xy + _ParallaxMap_ST.zw;
				float temp_output_155_0 = ( _Parallax * -1.0 );
				float3 ase_worldTangent = i.ase_texcoord3.xyz;
				float3 ase_worldNormal = i.ase_texcoord4.xyz;
				float3 ase_worldBitangent = i.ase_texcoord5.xyz;
			...
			// End Injection EMISSION from Injection_Emission_Meta.hlsl ----------------------------------------------------------
				...
				half3 emission = ( temp_output_249_0 * saturate( pow( abs( dotResult276 ) , ( _EmissionFalloff * 1.0 ) ) ) ).rgb;
				half3 bakedemission = ( temp_output_249_0 * _BakedMutiplier ).rgb;
...
/*ASEBEGIN
Version=18800
219;471;1219;463;1612.336;495.7229;1;True;False
...
Node;AmplifyShaderEditor.RangedFloatNode;120;-2734.739,1011.063;Inherit;False;Property;_Parallax;Height Scale;11;0;Create;False;0;0;0;False;0;False;0;-0.001;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.TexturePropertyNode;154;-2800.55,788.4324;Inherit;True;Property;_ParallaxMap;Height Map;10;1;[Header];Create;False;1;Parallax;0;0;False;0;False;None;f0d298cf35e051c40a59da46a43b224f;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
...
Node;AmplifyShaderEditor.TextureCoordinatesNode;434;-2493.347,672.8641;Inherit;False;0;154;4;3;2;SAMPLER2D;;False;0;FLOAT2;1,1;False;1;FLOAT2;0,0;False;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.CommentaryNode;205;-3125.834,-744.05;Inherit;False;1103.113;591.1926;;7;178;9;12;296;295;428;437;Albedo;1,1,1,1;0;0
...
Node;AmplifyShaderEditor.SamplerNode;9;-2786.547,-701.8194;Inherit;True;Property;_MainTex;Main Tex;1;0;Create;True;0;0;0;False;0;False;-1;None;d187569237abc6c4f8a9ff106a4c0426;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.ColorNode;12;-2688.582,-516.3932;Inherit;False;Property;_Color;Main Color;2;0;Create;False;0;0;0;False;0;False;1,1,1,0;0.74,0.74,0.74,0.4941176;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.TextureCoordinatesNode;164;-2508.666,1108.444;Inherit;False;0;165;4;3;2;SAMPLER2D;;False;0;FLOAT2;1,1;False;1;FLOAT2;0,0;False;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.TexturePropertyNode;165;-2972.001,1133.855;Inherit;True;Property;_DetailAlbedoMap;Detail Albedo X2;25;0;Create;False;0;0;0;False;0;False;None;8b40dec02df442549a13cb0a85e93156;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
...
Node;AmplifyShaderEditor.RangedFloatNode;440;-1065.64,-179.0131;Inherit;False;Property;_BlendDst;Blend Destination;57;1;[HideInInspector];Create;False;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;439;-1065.64,-108.2134;Inherit;False;Property;_ZWrite;ZWrite;52;2;[HideInInspector];[Toggle];Create;False;0;0;0;False;0;False;1;1;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;442;-933.1734,-105.6163;Inherit;False;Property;_Cull;Cull Side;59;1;[HideInInspector];Create;False;0;0;0;False;0;False;2;0;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;441;-1067.271,-252.4445;Inherit;False;Property;_BlendSrc;Blend Source;54;1;[HideInInspector];Create;False;0;0;0;False;0;False;1;1;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.IntNode;445;-1065.674,-325.6865;Inherit;False;Property;_Surface;Surface;55;1;[HideInInspector];Create;True;0;0;0;False;0;False;0;0;False;0;1;INT;0
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;443;-893.0255,-320.6774;Inherit;False;5;5;0;INT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;437;-2374.39,-486.3239;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;444;-798.3566,-134.7769;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;436;-317.8931,8.371094;Inherit;False;Property;_BakedMutiplier;Emission Baked Multiplier;15;0;Create;False;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;261;-3491.335,-407.7022;Inherit;False;Color Mask;-1;;130;eec747d987850564c95bde0e5a6d1867;0;4;1;FLOAT3;0,0,0;False;3;FLOAT3;0,0,0;False;4;FLOAT;1;False;5;FLOAT;1;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.ColorNode;186;-2436.659,-1300.002;Inherit;False;Property;_FluoresenceTint;Fluoresence Tint;32;1;[Header];Create;True;1;Fluoresence;0;0;False;0;False;1,1,1,1;1,1,1,1;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.Vector3Node;423;1273.974,1609.389;Inherit;False;Property;_g_flReflectanceMin;g_flReflectanceMin;58;0;Create;True;0;0;0;False;0;False;0,0,0;1,1,1;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
...
Node;AmplifyShaderEditor.SimpleAddOpNode;307;-580.1635,-123.0403;Inherit;False;3;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;163;-239.7593,126.9572;Inherit;False;Property;_Cutoff;Alpha Clipping;0;0;Create;False;0;0;0;False;0;False;0.5;0;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.SamplerNode;70;-2312.439,153.9961;Inherit;True;Property;_BumpMap;Normal Map;3;0;Create;False;0;0;0;False;0;False;-1;None;d738e924c6af4e2468162500a631a25e;True;0;True;bump;Auto;True;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.RangedFloatNode;23;-795.6489,536.092;Inherit;False;Property;_Metallic;Metallic;8;0;Create;True;0;0;0;False;0;False;1;0.132;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.SamplerNode;99;781.5314,137.8232;Inherit;True;Property;_DetailNormalMap;Detail Normal;26;1;[NoScaleOffset];Create;False;0;0;0;False;0;False;-1;None;d738e924c6af4e2468162500a631a25e;True;0;True;bump;Auto;True;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SamplerNode;21;-1443.805,239.8978;Inherit;True;Property;_MetallicGlossMap;Metallic Map;7;0;Create;False;0;0;0;False;0;False;-1;bfeb91aff4ad47443a4ab34d16f4ff0a;f5b3d9639e5b7ee45ae6126098f639bb;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SamplerNode;27;-1142.964,1539.424;Inherit;True;Property;_OcclusionMap;Occlusion Map;17;1;[Header];Create;True;1;Occlusion;0;0;False;0;False;-1;None;None;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.RangedFloatNode;419;939.2321,1573.466;Inherit;False;Property;_g_flSpecularPower;g_flSpecularPower;56;0;Create;True;0;0;0;False;0;False;0;1;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.StaticSwitch;169;-812.7413,106.8069;Inherit;False;Property;_MetallicType;Metallic Type;6;0;Create;True;0;0;0;False;0;False;0;0;1;True;;KeywordEnum;2;MAS;MetallicSmoothness;Reference;157;True;True;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;435;-59.57576,-25.01909;Inherit;False;2;2;0;COLOR;0,0,0,0;False;1;FLOAT;0;False;1;COLOR;0
...
Node;AmplifyShaderEditor.RangedFloatNode;415;703.8481,1598.776;Inherit;False;Property;_g_flReflectanceMax;g_flReflectanceMax;53;0;Create;True;0;0;0;False;0;False;0;1;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;100;515.1421,289.3925;Inherit;False;Property;_DetailNormalMapScale;Detail Normal Scale;27;0;Create;False;0;0;0;False;0;False;1;0.5;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;26;-756.3005,623.4407;Inherit;False;Property;_Glossiness;Smoothnes;9;0;Create;False;0;0;0;False;0;False;1;0.1;0;1.5;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.TemplateMultiPassMasterNode;400;113.2767,-59.96434;Float;False;True;-1;2;UnityEditor.LitMASIMGUI;0;12;Valve/VRStandard Nuevo;623634af11bd9ab448550ee777f3493e;True;Forward;0;0;Forward;14;True;1;1;False;441;0;False;440;0;1;False;-1;0;False;-1;False;False;False;False;False;False;False;False;True;0;False;-1;True;True;True;True;True;0;False;-1;False;False;False;True;False;255;False;-1;255;False;-1;255;False;-1;7;False;-1;1;False;-1;1;False;-1;1;False;-1;7;False;-1;1;False;-1;1;False;-1;1;False;-1;True;1;False;439;True;3;False;-1;True;True;0;False;-1;0;False;-1;True;3;RenderPipeline=UniversalPipeline;RenderType=Opaque=RenderType;Queue=Geometry=Queue=0;False;0;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;False;255;False;-1;255;False;-1;255;False;-1;7;False;-1;1;False;-1;1;False;-1;1;False;-1;7;False;-1;1;False;-1;1;False;-1;1;False;-1;False;False;False;True;1;Lightmode=UniversalForward;True;7;0;Hidden/InternalErrorShader;0;0;Standard;24;Workflow;1;Surface;0;Two Sided;1;Cast Shadows;1;  Use Shadow Threshold;0;GPU Instancing;0;Built-in Fog;1;Lightmaps;1;Volumetrics;1;Decals;0;Write Depth;0;  Early Z (broken);0;Vertex Position,InvertActionOnDeselection;1;Emission;1;PC Reflection Probe;3;PC Receive Shadows;1;PC Vertex Lights;0;PC SSAO;1;Q Reflection Probe;0;Q Receive Shadows;0;Q Vertex Lights;1;Q SSAO;0;Environment Reflections;1;Meta Pass;1;0;5;True;True;True;True;True;False;;False;0
...
WireConnection;443;2;441;0
WireConnection;443;3;439;0
WireConnection;443;4;442;0
...
WireConnection;437;0;9;4
WireConnection;437;1;12;4
...
WireConnection;307;1;437;0
WireConnection;307;2;444;0
...
ASEEND*/
//CHKSM=15CD661D8C61C069973A900FE2363C9C1F8A742F
