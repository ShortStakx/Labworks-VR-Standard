...
#if !defined(EVRO_PBR_LIGHTING_SSR) && !defined(SHADER_API_MOBILE)
#define EVRO_PBR_LIGHTING_SSR
...
real4 SLZPBRFragmentSSR(SLZFragData fragData, SLZSurfData surfData, SSRExtraData ssrExtra, int surfaceType = 0)
{
    ...
    float3 output = surfData.occlusion * diffuseAlbedo + surfData.emission;
...
//#endif
	...
	//	finalColor.rgba = ApplyFog( outputColor.rgba, i.vFogCoords.xy, _FogMultiplier, _ColorMultiplier );
	//	#else
	//	finalColor.rgb = ApplyFog( outputColor.rgb, i.vFogCoords.xy, _FogMultiplier );
	//	#endif
	//}
	//#endif
