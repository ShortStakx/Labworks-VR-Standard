...
/* 
 * VR Standard Edit of Stress Level Zero Lighting functions
 */   

#ifndef EVRO_PBR_LIGHTING
#define EVRO_PBR_LIGHTING
...
void EVROApplyFluorescence(half4 diffuse, inout half4 finalDiffuse, SLZSurfData surfData)
{
    #if defined(_FLUORESCENCEMAP)
        float4 FluorescenceAbsorb = diffuse * surfData.absorbance;					
        ...
        float3 LitFluorescence =  float3(Absorbed_R, Absorbed_G, Absorbed_B) * surfData.fluorescence.rgb ;
        finalDiffuse.rgb = max(finalDiffuse.rgb, LitFluorescence.rgb);
    #endif
}
...
half4 SLZPBRFragment(SLZFragData fragData, SLZSurfData surfData, int surfaceType = 0)
{
    ...
    float4 diffuseAlbedo = (surfData.albedo * diffuse);

    #if defined(_FLUORESCENCEMAP)
    EVROApplyFluorescence(diffuse, diffuseAlbedo, surfData);
    #endif
    
    half3 finalDiffuse = surfData.occlusion * diffuseAlbedo + surfData.emission;
